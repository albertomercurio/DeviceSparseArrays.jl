function shared_test_matrix_sell(
    op,
    array_type::String,
    int_types::Tuple,
    float_types::Tuple,
    ...
    @testset "DeviceSparseMatrixSELL $array_type" verbose=true begin
        shared_test_conversion_matrix_sell(
            op,
            array_type,
            int_types,
            float_types,
        ...
        shared_test_linearalgebra_matrix_sell(
            op,
            array_type,
            int_types,
            float_types,
...
function shared_test_conversion_matrix_sell(
    op,
    array_type::String,
    int_types::Tuple,
    float_types::Tuple,
    ...
    @testset "Conversion" begin
        ...
        # test only conversion SparseMatrixCSC <-> DeviceSparseMatrixSELL
        if op === Array
            dA = DeviceSparseMatrixSELL(A)
            @test size(dA) == (0, 0)
            @test length(dA) == 0
        ...
        B_csr_t = SparseMatrixCSC(transpose(B))
        dB = adapt(op, DeviceSparseMatrixSELL(transpose(B_csr_t)))
        dB2 = adapt(op, DeviceSparseMatrixSELL(B))
        @test size(dB) == (2, 2)
        @test length(dB) == 4
        @test nnz(dB) == 3
        # There is no easy way to test the internal storage of SELL here, so we use sums
        ...
        @test_throws ArgumentError DeviceSparseMatrixSELL(
            2,
            2,
            op(int_types[end][1, 3]),
            op(int_types[end][1, 2]),
...
function shared_test_linearalgebra_matrix_sell(
    op,
    array_type::String,
    int_types::Tuple,
    float_types::Tuple,
    ...
    #        # Convert to SELL storage pattern
    #        dA = adapt(op, DeviceSparseMatrixSELL(A))
    #
    #        @test sum(dA) ≈ sum(A)
    #        
    ...
    @testset "Matrix-Vector multiplication" begin
        for T in (int_types..., float_types..., complex_types...)
            ...
            for (op_A, op_B) in ((identity, identity),)
                ...
                dB = op(B)
                #dC = op_A(dA) * op_B(dB)
                dC = similar(dB, size(C)...)
                mul!(dC, op_A(dA), op_B(dB))
                @test collect(dC) ≈ C
