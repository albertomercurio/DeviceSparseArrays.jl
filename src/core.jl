...
const trans_adj_wrappers_dense_vecormat = (
    (T -> :(DenseVecOrMat{$T}), false, identity, identity, T -> :(DenseVecOrMat{$T})),
    (
        T -> :(Transpose{$T,<:DenseVecOrMat{$T}}),
        true,
        identity,
        A -> :(parent($A)),
        T -> :(DenseVecOrMat{$T}),
    ),
    (T -> :(Adjoint{$T,<:DenseVecOrMat{$T}}), true, x -> :(conj($x)), A -> :(parent($A))),
)
